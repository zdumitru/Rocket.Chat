...
Meteor.methods({
	setupImporter(name) {
		...
		if (Importer.Importers && Importer.Importers[name] && Importer.Importers[name].importer) {
			...
			if (importer.importerInstance) {
				...
				importer.importerInstance = new importer.importer(importer.name, importer.description, importer.mimeType);//eslint-disable-line
				return importer.importerInstance.getProgress();
			}
		} else {
