...
Template.loginFooter.onCreated(function() {
	...
	if (userLanguage !== defaultUserLanguage()) {
		return TAPi18n._loadLanguage(defaultUserLanguage()).done(() => self.languageVersion.set(defaultUserLanguage()));
	} else if (userLanguage.indexOf('en') !== 0) {
		return TAPi18n._loadLanguage('en').done(() => self.languageVersion.set('en'));
	}
});
